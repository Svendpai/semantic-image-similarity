...
class PretrainedMobileNetV2 implements IImageSimilarityCalculator {
    ...
    constructor() {
        ...
        this.calculateSimilarity = async (image1: string, image2: string) => {
            if (this.model) {
                ...
                const image1LightLevel = await calculateLightLevel(decodedImage1);
                const image2LightLevel = await calculateLightLevel(decodedImage2);
...
export default PretrainedMobileNetV2;
