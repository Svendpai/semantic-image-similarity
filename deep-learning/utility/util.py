...
def evaluate(data, labels, similarity, predictions = None, name="N/A", threshold=None):
    ...
    def visualize_data(final_data_list, to_show=10, offset=0):
        """Visualize a list of image pairs, that are sorted by similarity"""
...
def get_similarity_from_embedding(embedding, data):
    """Calculates the similarity of two image embeddings over a dataset"""
    ...
    for i in range(len(data)):
        features_1 = embedding.predict(np.expand_dims(data[i][0], axis=0))
        features_2 = embedding.predict(np.expand_dims(data[i][1], axis=0))
